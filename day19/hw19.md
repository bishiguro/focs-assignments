...
[x] I completed this assignment with assistance from ___
   and/or using these external resources: https://en.wikipedia.org/wiki/Dynamic_programming
```
...
1. ∃x. ∀y. loves(x, y)  
Translation: There exists some x which all y loves.  
No, there is no value for x that loves a, b, c, and d.  

2. ∃y. ∀x. loves(x, y)  
Translation: There exists some y which all x loves.  
Yes, a, b, c, and d all love c.  

3. ∀x. ∃y. loves(x, y)  
Translation: For all x, there exists a loved y.  
Yes, a, b, c, and d all love.  

4. ∀y. ∃x. loves(x, y)  
Translation: For each y, there exists an x who loves it.  
Yes, a, b, c, and d are all loved.  

5. ∃x. ∃y. loves(x, y)  
Translation: For some x, there is some y who loves it.  
Yes, love exists among pairings of a, b, c, and d.  

6. ∃x. ∀y. ¬loves(x, y)  
Translation: There is no x that loves all y.  
Yes, there is no x that loves a, b, c, and d.  

7. ∃x. ¬∀y. loves(x, y)  
Translation: There exists some x who does not love all y.  
Yes, none of a, b, c, or d love all y.  
...
2. sum(s(0), sum(0), s(s(0)))   1, Axiom 2
```
...
```
See max_subarray.pdf.
```

b. Is `max_subarray` an example of divide and conquer? Why or why not?

```
No, this algorithm is not recursive, and the sub-problems are not joined together to create the final solution.
```

c. Is `max_subarray` an example of dynamic programming? Why or why not?
```
Yes, the algorithm updates the max_ending_here and max_so_far values, keeping track of past calculations as it traverses the array. 
```
...
```
See binary_search_array.pdf.
```

b. Is `binary_search_array` an example of divide and conquer? Why or why not?

```
Yes, this algorithm splits the array to compute smaller sub-problems.
```

c. Is `binary_search_array` an example of dynamic programming? Why or why not?

```
No, the results of the smaller sub-problems are not saved for future reference.
```
